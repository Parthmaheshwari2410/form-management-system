...
export default function AddMemberAgent() {
    const [form, setForm] = useState({
        name: '', village: '', taluka: '', district: '', state: '', pincode: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await api.post('/agent/members', fd);
            setMsg(data.message);
            setForm({ name: '', village: '', taluka: '', district: '', state: '', pincode: '', email: '', whatsapp: '', contact: '', field_area_meter: '', field_area_acre: '', field_area_gunta: '', field_8a_info: '' });
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6 space-y-6">
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        ['state', 'State *'], ['pincode', 'PinCode *'], ['email', 'Email ID'], ['whatsapp', 'WhatsApp'], ['contact', 'Contact']
                        ].map(([field, label]) => (
                            <div key={field}>
                                <label className={lbl}>{label}</label>
                                <input type={field === 'email' ? 'email' : 'text'} value={form[field]}
                                    required={label.includes('*')}
                                    onChange={e => setForm({ ...form, [field]: e.target.value })} className={inp} />
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div>
                    <h3 className="font-semibold text-gray-700 border-b pb-2 mb-3">Documents</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[['aadhaar_doc', 'Aadhaar'], ['pan_doc', 'PAN'], ['doc_712', '7/12 Document'], ['doc_8a', '8A Document']].map(([k, l]) => (
                            <div key={k}>
                ...
                <div>
                    <h3 className="font-semibold text-gray-700 border-b pb-2 mb-3">Land Area</h3>
                        ...
                        {[['field_area_meter', 'Meter'], ['field_area_acre', 'Acre'], ['field_area_gunta', 'Gunta']].map(([k, l]) => (
                            <div key={k}><label className={lbl}>{l}</label>
                                <input type="number" step="0.01" value={form[k]}
                                    onChange={e => setForm({ ...form, [k]: e.target.value })} className={inp} /></div>
                        ))}
                    </div>
                    <div className="mt-3">
                        <label className={lbl}>8A Info</label>
                        <textarea rows={3} value={form.field_8a_info}
                            onChange={e => setForm({ ...form, field_8a_info: e.target.value })} className={inp} />
                    </div>
                </div>
